...
export const AllArticulosOnCategory = ({
  initialProductos,
  totalProductos,
  name_category
}: any) => {
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      const response = await fetch(
        `${Global.url}/newsletter/getAllArticulosPublic/${name_category}?offset=${offset}&limit=12`
      )
      if (!response.ok) {
  ...
  return (
    ...
    >
      {productos.length == 0 && !loading && (
        <p className='text-gris'>No hay artículos disponibles.</p>
      )}
      {productos.map((producto: any, index: number) => {
        ...
        if (contador == 1) {
                  ...
                  {index == 0 && (
                    <>
                      ...
                      <div className="mt-4 mb-4 px-4 lg:px-0">
                        <h2 className="text-3xl lg:text-5xl font-ysobel_regular tracking-wider first-letter:uppercase lowercase ">
                          {producto.categoria}
                        </h2>
                      </div>
                    </>
                  )}
                  <div className="h-[300px] lg:h-[450px] w-full relative">
                    <Skeleton
                      variant="rectangular"
          ...
          return (
            <Link
              href={`/newsletter/${producto.url_categoria}/${producto.url}`}
              className="flex w-full flex-col text-xs col-span-2 lg:col-span-1"
              key={index}
            >
          ...
          return (
            <Link
              href={`/newsletter/${producto.url_categoria}/${producto.url}`}
              className="flex w-full flex-col text-xs mt-4 lg:mt-0"
              key={index}
            >
              <div className="text-xs text-gray-600 px-4 lg:px-0">
                <FormatoFecha fechaISO={producto.created_at} />
          ...
          return (
            <Fragment key={index}>
              ...
              <Link
                href={`/newsletter/${producto.url_categoria}/${producto.url}`}
                className="flex w-full mt-6 lg:mt-10 justify-between items-center col-span-2 lg:col-span-3 lg:items-start text-xs px-4 lg:px-0"
              >
