...
import { AllArticulos } from './@components/arituculos/AllArticulos'
...
export default async function Newsletter () {
  ...
  const categoriasRes = await fetch(`${Global.url}/newsletter/getAllCategoriasPublic`, { cache: 'no-store' })

  const productosRes = await fetch(`${Global.url}/newsletter/getAllArticulosPublicInitial`, { cache: 'no-store' })

  const getAllArticulosPublicSection = await fetch(`${Global.url}/newsletter/getAllArticulosPublicSection`, { cache: 'no-store' })
  ...
  return (
    <section className="flex pt-24 relative max-w-[1700px] mx-auto">
      <a
        ...
        rel="noreferrer"
      >
        <img src="/assets/images/newsletter/anuncio1.webp" alt="Logos Perú" className="w-full h-full object-contain object-right-top " />
      </a>
      <section className="w-full grid grid-cols-1 lg:grid-cols-8 flex-1 relative bg-white">
        <a
          ...
          rel="noreferrer"
        >
          <img src="/assets/images/newsletter/anuncio3.webp" alt="Logos Perú" className="h-[100px] w-full object-cover lg:hidden" />
        </a>
        <div className="w-fulll col-span-1 border-r-2 border-l-2 border-gris h-full p-8 2xl:p-10 hidden lg:block">
          <h2 className="text-3xl border-b-2 border-b-gray-500 w-fit font-ysobel_regular">Todos</h2>
          <div className="mt-6 font-ysobel_regular flex flex-col gap-3 text-gray-600">
            {categorias?.map((categoria: any) => (
              <Link href={`/newsletter/${categoria.url}`} key={categoria.url} className="lowercase first-letter:uppercase">
                {categoria.titulo}
              </Link>
            ))}
          </div>
        ...
        <Suspense fallback={<Loading />}>
          <AllArticulos initialProductos={data} totalProductos={total} />
        </Suspense>
      ...
      <a
        ...
        rel="noreferrer"
      >
        <img src="/assets/images/newsletter/anuncio2.webp" alt="Logos Perú" className="w-full h-full object-contain object-left-top" />
      </a>
    </section>
  )
}
