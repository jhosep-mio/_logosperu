...
export const SchemaForm = Yup.object().shape({
  ...
  celular: Yup.string().required('Este campo es requerido').min(9, 'Debe tener como minimo 9 digitos').max(255, 'Debe tener menos de 255 caracteres'),
...
export const SchemaBoletin = Yup.object().shape({
  nombre: Yup.string().required('Este campo es requerido').min(3, 'Debe tener como minimo 3 digitos').max(255, 'Debe tener menos de 255 caracteres'),
  email: Yup.string().required('Este campo es requerido').email('Introduce un email válido').max(255, 'Debe tener menos de 255 caracteres')
})
...
export const SchemaPago = Yup.object().shape({
  ...
  pais: Yup.string().required('Este campo es requerido').max(20, 'Debe tener menos de 20 caracteres'),
  ...
  email: Yup.string().required('Este campo es requerido').email('Introduce un email válido').max(30, 'Debe tener menos de 20 caracteres'),
