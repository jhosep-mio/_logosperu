...
export async function SeoOneNesletter ({ params }: any) {
  ...
  function getFirstParagraph (htmlString: string) {
    const match = htmlString.match(/<p>(.*?)<\/p>/i) // Buscar el primer <p>
    return match ? stripHTML(match[1]) : '' // Retornar el contenido del <p> sin HTML
  }
  const seoDescription = `${getFirstParagraph(producto[0].contenido)} | Logos Peru Agencia de diseño de logos, logotipos, Diseño Grafico y Desarrollo Web.`

  return {
    title: `${producto[0].titulo} | Logos Perú `,
    description: seoDescription,
    alternates: {
      canonical: `https://logosperu.com.pe/newsletter/${name_category}/${name}`
    },
    ...
    openGraph: {
      title: `${producto[0].titulo} | Logos Perú `,
      description: seoDescription,
      url: `https://logosperu.com.pe/newsletter/${name_category}/${name}`,
      site_name: 'Logos Perú',
      type: 'website',
      images: [
        {
          url: `https://mysistemalogosperu.s3.amazonaws.com/articulos/${producto[0].imagen1}`
        }
      ]
    },
    twitter: {
      ...
      description: seoDescription,
      creator: '@DLogosPeru',
      image: `https://mysistemalogosperu.s3.amazonaws.com/articulos/${producto[0].imagen1}`
    },
