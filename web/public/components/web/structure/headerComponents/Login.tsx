...
export const Login = ({
  ...
  const validar = async (values: Values): Promise<void> => {
    ...
    try {
      ...
      if (respuesta.data.status == 'success') {
        ...
        if (respuesta.data.message == 'Admin') {
          const tokenData = {
            token: `q16${respuesta.data.access_token}90x`
          }
          ...
          const tokenEncoded = encodeURIComponent(encryptedToken)
          const redirectTo = `https://intranet.logosperu.com.pe/auth?tk=${tokenEncoded}`

          window.location.href = redirectTo
        } else if (respuesta.data.message == 'Usuario') {
          //   const tokenData = {
          //     token: `q16${respuesta.data.access_token}90x`
          //   }
          //   const tokenString = JSON.stringify(tokenData)
          //   const encryptedToken = CryptoJS.AES.encrypt(tokenString, 'secret').toString()
          //   const tokenEncoded = encodeURIComponent(encryptedToken)
          //   const redirectTo = `https://intranet.logosperu.com.pe/auth?tk=${tokenEncoded}`
  ...
  const { handleSubmit, handleChange, errors, values, touched, handleBlur, resetForm } =
    useFormik({
      initialValues: {
        email: '',
        password: ''
