...
export const Register = ({ open, setOpen, setOpenLogin }: any) => {
    ...
    if (values.code.length == 6) {
      ...
      try {
        ...
        if (respuesta.data.status == 'success') {
          ...
          setOpen(false)
          setOpenLogin(true)
        } else if (respuesta.data.status == 'error' && respuesta.data.message == 'Código de verificación inválido o expirado') {
  ...
  const { handleSubmit, handleChange, setValues, resetForm, errors, values, touched, handleBlur } =
    useFormik({
      initialValues: {
        nombres: '',
        apellidos: '',
            ...
            <>
              ...
              <div>
                ...
                <button
                  type='button'
                  disabled={loading}
                  onClick={() => { setOpenCode(false); setValues({ ...values, code: '' }) }}
