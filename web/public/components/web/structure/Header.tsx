...
import { ResetPassword } from './headerComponents/ResetPassword'
...
import { ModalSuscribirse } from '@/app/newsletter/@components/modals/ModalSuscribirse'
...
export const Header = () => {
  ...
  const headerClass =
    scroll > 0 ? 'bg-transparent py-2 shadow-lg' : 'py-3 lg:pt-4'
  return (
    <>
      <Toaster position="top-center" richColors />

      <header
        className={`${headerClass} fixed left-0 top-0 z-[999] w-full px-2 transition-all lg:px-16`}
        style={{ backdropFilter: scroll > 0 ? 'blur(8px)' : 'blur(0px)' }}
      >
        <nav className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-0">
            <button className="bg-[#2563EB] hidden text-xs xl:text-sm font-bold justify-center text-white py-0 px-6 rounded-full hover:bg-blue-600 lg:flex items-center gap-2 transition duration-300">
              <img
            ...
            <div className="bg-[#2563EB] text-xs cursor-pointer lg:text-lg lg:hidden justify-center text-white py-0 px-4 rounded-full hover:bg-blue-600 flex items-center gap-2 transition duration-300">
              <img
                src="https://img.icons8.com/color/48/visa.png"
                alt="Visa"
                className="w-full mx-auto object-contain h-8"
              />
            </div>
            {auth.id ? (
              <div
