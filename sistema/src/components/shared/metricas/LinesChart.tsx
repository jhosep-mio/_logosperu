...
export default function LinesChart (): JSX.Element {
  ...
  const [dataMedios, setdataMedios] = useState<any>({
    labels: [],
    datasets: [
      {
  ...
  const todosLosGeneros = (): void => {
    if (clientes.length > 0) {
      ...
      clientes.forEach(
        (cliente: { region?: string, edad: number, sexo: string }) => {
          if (!cliente.region) {
            return
          }
          const clienteObj = JSON.parse(cliente.region)
  ...
  const todosLosMedios = (): void => {
    if (clientes.length > 0) {
      ...
      clientes.forEach((orden: { region?: string, medio_ingreso: string }) => {
        // Convertir medio_ingreso a string, si no lo es
        if (!orden.region) {
          return
        }
        const clienteObj = JSON.parse(orden.region)
        if (!clienteObj) {
          return // Si no se puede parsear la ubicación, omitirlo
        }
        const country = clienteObj.country
        ...
        const medio_ingreso = String(orden.medio_ingreso)
        ...
        datasets: [
          {
            label: 'Cantidad de Personas por Medio de Ingreso',
            data: sortedData
            // ... (otras propiedades del conjunto de datos, como colores, etc.)
          }
        ]
      }

      setdataMedios(chartData) // Asegúrate de tener un estado o una función setDataMedio para actualizar los datos del gráfico
    }
  }
