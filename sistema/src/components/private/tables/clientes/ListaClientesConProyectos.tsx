...
export const ListaClientesConProyectos = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de clientes con proyectos')
    Promise.all([
      getClientes('indexProyectos'),
    ...
    <>
      ...
      {loading
        ...
        <div className="md:bg-[#fff] p-0 md:p-8 rounded-xl">
          <div className="hidden md:grid grid-cols-1 md:grid-cols-10 gap-3 mb-2 md:px-4 md:py-2 text-gray-400 border-y border-gray-300">
            ...
            <h5 className="md:text-center line-clamp-1">C. Proyectos</h5>
          ...
          {filterDate().map(
            (orden: ValuesPreventaModificate, index: number) => (
              <>
                <div
                  className={`grid grid-cols-1 md:grid-cols-10 relative gap-3 items-center  mb-3 md:mb-0 ${
          ...
          <ModalPreviaCotizacion open={openCotizacion} setOpen={setOpenCotizacion} planes={planes}
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error
          datos={values}/>
