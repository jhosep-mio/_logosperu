...
export const ListaBriefDiseñoNew = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Lista de Briefs - Logotipo')
    roles.map(
      (rol: RolsValues): void =>
        rol.id == auth.id_rol &&
        JSON.parse(rol.accesos).map(
          async (route: { peso: string }): Promise<void> => {
            if (route.peso == 'superusuario') {
              await Promise.all([
                getDataNew('getBriefsDiseño', setProductos, setTotalRegistros)
              ]).then(() => {
                setLoading(false)
              })
            } else {
              await Promise.all([
                getDataNew(
                  `getBriefsDiseñoAsignacion/${auth.id}`,
                  setProductos,
                  setTotalRegistros
                )
              ]).then(() => {
    ...
    <>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-y-4 mb-5">
        ...
        <div className="w-full md:w-fit flex flex-col-reverse md:flex-row items-center gap-4">
          <Link
            to={'/admin/lista-briefs-diseños-new/agregar'}
      ...
      {loading
        ...
        <div className="md:bg-[#fff] md:p-8 rounded-xl">
          ...
          {filterDate().map((orden: interfaceListaDiseñoNew) => (
            <div
              className="grid grid-cols-1 md:grid-cols-8 gap-4 items-center mb-4 bg-white md:bg-gray-50 p-4 rounded-xl shadow-sm cursor-pointer"
              key={orden.id}
            >
              ...
              <div className="md:text-center md:flex md:justify-center items-center">
                ...
                >
                  <MenuItem className="p-0 hover:bg-transparent">
                    {orden.id != null && (
                      <Link
                        to={`/admin/lista-briefs-diseños-new/view/${orden.id}`}
                        ...
                        (route: { peso: string }) =>
                          ...
                          orden.id !== null && (
                            <>
                              <MenuItem className="p-0 hover:bg-transparent">
                                <Link
                                  to={`/admin/lista-briefs-diseños-new/asignar/${orden.id}`}
                                  className="rounded-lg transition-colors text-gray-300 hover:bg-secondary-900 flex items-center gap-x-4 p-2 flex-1"
                                >
                                  Asignar Diseñador
                                </Link>
                              ...
                              <MenuItem className="p-0 hover:bg-transparent">
                                <Link
                                  to={`/admin/lista-briefs-diseños-new/editar/${orden.id}`}
                                  className="rounded-lg transition-colors text-gray-300 hover:bg-secondary-900 flex items-center gap-x-4 p-2 flex-1"
                                >
                                  Editar
                                </Link>
