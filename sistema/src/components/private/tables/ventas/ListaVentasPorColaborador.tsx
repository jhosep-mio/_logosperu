...
import SelectFilter from './filtros/SelectFilter'
...
export const ListaVentasPorColaborador = (): JSX.Element => {
    ...
    <>
      <div className="flex flex-col lg:flex-row items-center justify-between gap-y-4 mb-3 lg:mb-5 gap-2">
        ...
        <div className="w-fit lg:w-fit flex flex-row items-center gap-4">
          ...
          <Link to={'status'}>
            <FaComments className="text-green-700 text-2xl lg:text-4xl cursor-pointer" />
          </Link>
          <div className="w-fit lg:w-fit flex flex-row items-center gap-4">
      ...
      {loading
        ...
        <div className="lg:bg-[#fff] lg:px-8 lg:py-6 rounded-xl">
          <div
            className={`hidden items-center lg:grid pr-10 lg:pr-4 grid_teplate_ventas ${
              filters.enCola ? 'grid_teplate_ventas_encola' : ''
            } ${
              filters.sinFechaFinYNo1 ? 'grid_teplate_ventas_pendientes' : ''
            } 
            ${area === 'community' && (filters.fechaFin == null) ? 'grid_community_activo' : ''} 
            ${area === 'community' && (filters.estado != null) ? 'grid_community_activo_with_abandono' : ''} 
            ${area === 'community' && filters.activeFilter === 'sinFechaFinYNo1' ? 'grid_community_activo_with_proceso' : ''} 
          ...
          >
            ...
            {(area != 'community' || filters.fechaFin != null) &&
          ...
          {filterDate().map((orden: ValuesVenta, index: number) => (
            <div
              ...
              ${obtenerDiasParaPrimerPago(orden.fecha_inicio) > 0 && obtenerDiasParaPrimerPago(orden.fecha_inicio) <= 2 && area === 'community'
              ? 'casi_vencimiento_community'
              : `${obtenerDiasParaPrimerPago(orden.fecha_inicio) == 0 && area === 'community' ? 'vencimiento_community' : ''}`}
