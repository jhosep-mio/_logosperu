...
export const ListaProyectosAgencia = (): JSX.Element => {
  ...
  const getResumen = async (): Promise<void> => {
    try {
      ...
      if (request.data[0].resumen_adicional && JSON.parse(request.data[0].resumen_adicional).length > 0) {
            ...
            <AnimatePresence>
              {openEstado && (
                <motion.div
                  ...
                  className="flex flex-col gap-2 h-40  rounded-xl z-10 absolute mt-3 top-full inset-0"
                >
                  <button
