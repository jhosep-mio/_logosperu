...
export const ModalContenido = ({
  ...
  const [contenidoGestor, setContenidoGestor] = useState<any | null>(null)
  ...
  // CREAR CONTENIDO
  const crearContenido = (): void => {
    if (contenidoSeleccionado?.contenido.contexto == null) {
      const cod = uuidv4()
      updateIDItemTarjeta(contenidoSeleccionado?.contexto.id, contenidoSeleccionado?.contenido?.id, cod, contenidoSeleccionado?.contenido?.titulo)
    }
  }
  ...
  const getContenido = async (): Promise<void> => {
    setLoading(true)
    try {
      const { data } = await axios.get(`${Global.url}/gestorContenido/findOneContenido/${contenidoSeleccionado?.contenido.contexto ?? ''}`, {
        headers: {
  ...
  useEffect(() => {
    if (open && contenidoSeleccionado?.contenido.contexto != null) {
      getContenido()
    } else {
      setLoading(false)
    }
  ...
  return (
    ...
    >
      ...
      {contenidoSeleccionado?.contenido.contexto == null && contenidoGestor == null ? (
        <DialogContent className="w-full min-h-[600px] bg-[#F1F2F4] overflow-hidden flex items-center justify-center">
          <button
            className="bg-main text-black rounded-lg px-4 py-2 hover:bg-main_dark transition-colors"
            onClick={() => {
              crearContenido()
            }}
          >
          ...
          <div className="flex justify-between">
            <div className="flex gap-3 items-start w-[95%]">
              ...
              <div className="flex flex-col gap-2 w-full">
                <h1 className="text-lg text-gray-700 font-medium flex-1">{contenidoSeleccionado?.contenido.titulo}</h1>
