...
export const ModalFecha = ({
  ...
  startDate: Date | null
  updateDatos: any
  validacionShared: boolean
}): JSX.Element => {
  const [isCalendarOpen, setIsCalendarOpen] = useState(false)
  const datePickerRef = useRef(null)

  const handleDateChange = (date: Date | string): void => {
    updateDatos(undefined, undefined, undefined, undefined, date)
    closeCalendar()
  ...
  useEffect(() => {
    // Manejo de la tecla Escape para cerrar el calendario
      ...
      <div className="relative">
        {validacionShared ? (
          <>
            ...
            {startDate != null && (
              <div className="flex items-center w-full">
                <button
                  onClick={() => {
                    handleDateChange('null') // Eliminar la fecha
                  }}
                  className="text-white bg-red-600 hover:bg-red-800 transition-colors px-4 py-1 rounded-md mx-auto text-sm"
                >
                  Eliminar fecha
                </button>
              </div>
          ...
          <span className="text-gray-500 text-sm">
            Debe colocar la tarjeta en modo publico
          </span>
        )}
      </div>
    </div>
