...
import SelectFilterDesarrollo from '../ventas/filtros/SelectFilterDesarrollo'
...
export const ListaDesarrolloWeb = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de proyectos - Desarrollo Web')
    Promise.all([
      getColaboradores(),
      getUsuarios(),
      getDataToPlanes('getPlanes', setplanes, setTotalRegistros),
      getDataVentas('indexCapaDesarrollo', setProductos, setTotalRegistros)
    ]).then(() => {
      setLoading(false)
    })
  }, [])
  ...
  const filterDate = (): ValuesVenta[] => {
    ...
    filteredProductos = filteredProductos.filter(
      (pro) =>
      // @ts-expect-error
        pro.categoria_general == 'desarrollo'
    )
  ...
  function convertirFecha (fecha: any): any {
      ...
      {loading ? (
        ...
        <div className="lg:bg-[#fff] lg:px-8 lg:py-6 rounded-xl">
          ...
          >
            ...
            <h5 className="lg:text-center w-full line-clamp-1 gap-2 items-center ">
                F. Inicio
            </h5>
            {!filters.sinFechaFinYNo1 && (
                <h5 className="lg:text-center w-full line-clamp-1 gap-2 items-center col-span-2">
                F. Final
                </h5>
            )}
            ...
            {(area2 == 'desarrollo') &&
              ...
              filters.estado !== 1 && (
                <>
                  <h5 className="lg:text-left ">Sop {filters.estado}</h5>
                </>
            )}
          </div>
          {filterDate().map((orden: ValuesVenta, index: number) => (
            <>
              <div
                className={`grid grid_community_activo  ${
                  filters.enCola ? 'grid_teplate_ventas_encola' : ''
                } 
              ${
                filters.sinFechaFinYNo1 ? 'grid_teplate_ventas_pendientes' : ''
              ...
              >
                ...
                <div className="hidden lg:block lg:text-center ">
                    {orden.fecha_inicio && orden.fecha_fin && orden.estado == '0' ?
                        <span className='text-black'>{calcularDias(orden.fecha_inicio, orden.fecha_fin)} d</span>
                      : <span></span>
                    }
                </div>

                {// @ts-expect-error
                (orden?.categoria_plan == 'tienda' ||
                    // @ts-expect-error
                    orden?.categoria_plan == 'landing' ||
                    // @ts-expect-error
                    orden?.categoria_plan == 'administrable') && orden.fecha_fin != null ? (
                    ...
                    ) : <span className="text-[#252525] pl-2">{'-'} </span>
                }
