...
export const ListadoFlyers = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de Briefs - Flyer')
    roles.map(
      (rol: RolsValues): void =>
        rol.id == auth.id_rol &&
        JSON.parse(rol.accesos).map(
          async (route: { peso: string }): Promise<void> => {
            if (route.peso == 'superusuario') {
              await Promise.all([
                getDataNew('getBriefFlyer', setProductos, setTotalRegistros)
              ]).then(() => {
                setLoading(false)
              })
            } else {
              await Promise.all([
                getDataNew(
                  `getBriefFlyerAsignacion/${auth.id}`,
                  setProductos,
                  setTotalRegistros
                )
              ]).then(() => {
              ...
              <div className="md:text-center md:flex md:justify-center items-center">
                ...
                >
                  <MenuItem className="p-0 hover:bg-transparent">
                    {orden.id != null && (
                      <Link
                        to={`/admin/lista-briefs-flyer/view/${orden.id}`}
                        ...
                        (route: { peso: string }) =>
                          ...
                          orden.id !== null && (
                            <>
                              <MenuItem className="p-0 hover:bg-transparent">
                                <Link
                                  to={`/admin/lista-briefs-flyer/asignar/${orden.id}`}
                                  className="rounded-lg transition-colors text-gray-300 hover:bg-secondary-900 flex items-center gap-x-4 p-2 flex-1"
                                >
                                  Asignar Diseñador
                                </Link>
                              ...
                              <MenuItem className="p-0 hover:bg-transparent">
                                <Link
                                  to={`/admin/lista-briefs-flyer/editar/${orden.id}`}
                                  className="rounded-lg transition-colors text-gray-300 hover:bg-secondary-900 flex items-center gap-x-4 p-2 flex-1"
                                >
                                  Editar
                                </Link>
