...
import { type ValuesSubCategoriasPortafolio } from '../../../../shared/schemas/Interfaces'
...
import { DeleteItemsSubCategoriasToPortafolio } from '../../../../shared/functions/DeleteItemsCategoriasToPortafolio'

export const ListaSubCategoriasPortafolio = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de Subcategorias - Portafolio')
    Promise.all([
  ...
  const filterDate = (): ValuesSubCategoriasPortafolio[] => {
    if (search.length === 0) {
  ...
  const preguntar = (id: number): void => {
    ...
    DeleteItemsSubCategoriasToPortafolio({
      ruta: 'deleteSubCategoriaToPortafolio',
      id,
      token,
      totalPosts,
      cantidadRegistros,
      paginaActual,
      setpaginaActual,
      rutaFetch: 'getSubCategoriasToPortafolio',
      ...
      {loading
        ...
        <div className="bg-[#fff] p-3 md:p-8 rounded-xl">
          <div className="hidden md:grid grid-cols-1 md:grid-cols-5 gap-4 mb-10 p-4 rounded-md border-b-2 border-solid border-black-50">
            ...
            <h5 className="md:text-center text-black">URL</h5>
            <h5 className="md:text-center text-black">Opciones</h5>
          </div>
          {filterDate().map((orden: ValuesSubCategoriasPortafolio) => (
            <div
            ...
            >
              ...
              <div className="md:text-center md:flex md:justify-center">
                <h5 className="md:hidden text-black font-bold mb-2">Categoria</h5>
                <p className="line-clamp-2 text-black">{orden.categoria}</p>
              </div>

              <div className="md:text-center md:flex md:justify-center">
                <h5 className="md:hidden text-black font-bold mb-2">URL</h5>
                <p className="line-clamp-2 text-black">{orden.url}</p>
              </div>
