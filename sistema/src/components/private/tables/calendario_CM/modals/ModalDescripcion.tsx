...
import { type usurioValues, type arrayCategoriasToPortafolio, type comentariosValues } from '../../../../shared/schemas/Interfaces'
...
export const ModalDescripcion = ({
  ...
  const [arrayArchivos, setArrayArchivos] = useState<arrayCategoriasToPortafolio[]>([])
  ...
  function formatFileName (fileName: string): string {
    return fileName
      .normalize('NFD') // Descompone caracteres compuestos (ej: á -> a´)
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (file) {
      ...
      const uniqueFileName = `${uuidv4()}_${formatFileName(file.name)}`
  ...
  useEffect(() => {
    if (eventSelected?.event) {
      ...
      if (eventSelected?.event?.descripcion?.selectedContacts) {
        setSelectedContacts(eventSelected?.event?.descripcion?.selectedContacts)
      } else {
        setSelectedContacts([])
      }
      if (eventSelected?.event?.descripcion?.visitasInteracciones) {
        setVisitasInteracciones(eventSelected?.event?.descripcion?.visitasInteracciones)
      } else {
  ...
  const updateCita = async (updatedEvents: Event[]): Promise<void> => {
    ...
    try {
      ...
      if (respuesta.data.status == 'success') {
        ...
        formData.append('file_name', arrayArchivos[0].imagen1.archivoName)
        ...
        if (archivosAEliminar.length > 0) {
          ...
          for (const archivoName of archivosAEliminar) {
            await s3Client.send(
              new DeleteObjectCommand({
                Bucket: 'mysistemalogosperu',
                Key: `archivosComunnity/${archivoName}`
              })
            )
          }
        }
        // IMAGENES A SUBIR
        if (arrayArchivos[0]?.imagen1?.archivo && Object.keys(arrayArchivos[0].imagen1.archivo).length > 0) {
          const upload = new Upload({
            client: s3Client,
            params: {
              Bucket: data.bucket,
              Key: `archivosComunnity/${arrayArchivos[0].imagen1.archivoName}`,
              Body: arrayArchivos[0].imagen1.archivo ?? '',
  ...
  const handleInputChange = (type: string, field: string, value: string): void => {
    setVisitasInteracciones((prevData: any) => ({
      ...prevData,
      [type]: { ...prevData[type], [field]: value } // Actualiza el campo específico para el tipo dado
    }))
  }

  const obtenerURLCodificada = (archivoName: string): string => {
    const nombreCodificado = encodeURIComponent(archivoName).replace(/%20/g, '+') // Reemplaza los espacios codificados por '+'
  ...
  return (
    <>
      <Dialog
        ...
        maxWidth="xl"
      >
        <DialogContent className="w-[1000px] grid grid-cols-2 gap-4">
          <div className="w-full relative col-span-3 pb-4 border-gray-300 border-b-2">
            <h1 className="w-full uppercase text-center font-bold text-2xl">{eventSelected?.title}</h1>
          ...
          {eventSelected?.event?.estado == 'publicaciones' ||
          eventSelected?.event?.estado == 'history' ||
          eventSelected?.event?.estado == 'tiktok' ? (
            <div className="w-full">
              <EditorContexto content={contexto} setContent={setContexto} />
              {arrayArchivos?.length == 0 && (
                <div className="w-fit mt-4">
                  <div className="relative">
                    <button type="button" className="bg-red-500 text-white px-4 py-1 rounded-lg flex gap-3 text-sm items-center">
              ...
              <div className="w-full flex flex-col my-3 gap-6 justify-center flex-grow ">
                {arrayArchivos?.map((pro: any) => (
                  <div className="w-full" key={pro.id}>
                    <div className="group relative w-full" key={pro.id}>
                      {pro.imagen1.archivo != null && pro.imagen1.archivo.size > 0 ? (
                        pro.imagen1.archivo.type.includes('image') ? (
                          <RViewer imageUrls={`${URL.createObjectURL(pro.imagen1.archivo)}`}>
                            <RViewerTrigger>
                              <img src={`${URL.createObjectURL(pro.imagen1.archivo)}`} className=" mx-auto object-top w-full cursor-pointer " />
                            </RViewerTrigger>
                          </RViewer>
                        ) : (
                          <video src={`${URL.createObjectURL(pro.imagen1.archivo)}`} muted autoPlay loop className=" mx-auto object-top w-full " />
                        )
                      ) : (
                        pro.imagen1.archivo && (
                          <div className="w-full">
                            {isVideo(pro.imagen1.archivoName) ? (
                              <video
                                // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                                src={`https://mysistemalogosperu.s3.us-east-1.amazonaws.com/archivosComunnity/${obtenerURLCodificada(
                                  pro.imagen1.archivoName
                                )}`}
                                muted
                                autoPlay
                                loop
                                className=" mx-auto object-top w-full "
                              />
                            ) : (
                              <img
                                // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
              ...
              <div className="mt-4">
                <ul className="grid grid-cols-1 gap-2">
                  {(eventSelected?.event?.estado === 'publicaciones' || eventSelected?.event?.estado === 'history'
                    ? ['Facebook', 'Instagram']
                    : ['TikTok']
                  ).map((type) => (
                    <li key={type} className="flex items-center flex-col w-full bg-gray-100 rounded-lg p-2">
                      <span className="ml-2 text-base text-gray-600 font-bold uppercase">{type}</span>
                      <section className="w-full flex items-center mt-3">
                        {eventSelected?.event?.estado === 'history' ? (
                          <>
                            <div className="w-full px-2 gap-1 flex flex-col lg:relative pb-5">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  handleInputChange(type, 'visitas', e.target.value)
                                }}
                            ...
                            <div className="w-full px-2 gap-1 flex flex-col lg:relative pb-5">
                              <label
                                className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                                htmlFor="interacciones"
                              >
                                Interacciones
                              </label>
                              <input
                                type="text"
                                value={visitasInteracciones[type]?.megusta || 0}
                                onChange={(e) => {
                                  handleInputChange(type, 'megusta', e.target.value)
                                }}
                                className="flex h-9 w-full rounded-md border border-gray-400 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                              />
                            </div>
                          ...
                          <>
                            ...
                            <div className="w-full px-2 gap-1 flex flex-col lg:relative pb-5">
                              ...
                              <input
                                type="text"
                                placeholder="alcance"
                                value={visitasInteracciones[type]?.alcance || 0}
                                onChange={(e) => {
                                  handleInputChange(type, 'alcance', e.target.value)
                                }}
                                className="flex h-9 w-full rounded-md border border-gray-400 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                              />
                            </div>
                            ...
                            <div className="w-full px-2 gap-1 flex flex-col lg:relative pb-5">
                              ...
                              <input
                                ...
                                value={visitasInteracciones[type]?.impresiones || 0}
                                onChange={(e) => {
                                  handleInputChange(type, 'impresiones', e.target.value)
                                }}
                                className="flex h-9 w-full rounded-md border border-gray-400 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                              />
                            </div>
                        ...
                        {pro.arrayColaboradores.map((col: any, index: number) => (
                          ...
                          >
                            <p
                              ...
                              className={`w-full text-center cursor-pointer ${!col.cantidadVistas ? 'text-main' : 'text-black'}`}
                            >
                              {col.usuario.name}
                            </p>
          ...
          {openRegistro && (
            ...
            >
              <div className="w-[400px] h-fit p-6 bg-white rounded-lg shadow-md relative">
                ...
                <h2 className="text-xl uppercase mb-4 text-black w-full text-center font-bold">Editar visitas</h2>
                <select name="" id="" disabled value={selectedUser?.id} className="mb-4 w-full border border-gray-300 rounded-md px-4 py-2">
        ...
        <div className="w-full flex justify-start mb-3">
          {loadingUpdate ? (
            <button disabled className="w-fit mx-auto px-5 py-2 rounded-md bg-green-700 transition-colors text-white">
              Validando...
            </button>
          ) : (
            <button
