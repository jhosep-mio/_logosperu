...
export const ModalFeErratas = ({
  ...
  const [asunto, setAsunto] = React.useState('FE ERRATAS')
  ...
  React.useEffect(() => {
    ...
    // Formatear el día y el mes como strings con ceros a la izquierda si es necesario
    ...
    if (arrayAvances.length > 0) {
      const ultimoValor = arrayAvances[arrayAvances.length - 1]
      nombre = ultimoValor.asunto
    }
    const contextoG = `<p>FE DE ERRATAS</p><p><br></p><p>Mediante la presente se comunica a la Sr(a). ${datos?.nombres} "${datos?.nombre_marca}"; que el día hoy ${formattedDay} de ${formattedMonth} del presente año; se envió un correo de "${nombre ?? ''}" del proyecto en mención.</p><p><br></p><p>Se efectúa el fe de erratas.</p>`
    setContexto(contextoG)
  }, [open])

  // Formatear el día como un string con ceros a la izquierda si es necesario
