...
export const ListaVentas2 = ({
  ...
  const filterDate = (): ValuesVentaToMetricas[] => {
    if (open.proyectos) {
      let filteredProductos = open.proyectos
  ...
  return (
    <>
      ...
      {open.estado
        ? <>
        <div
          className={'bg-main/90 rounded-t-md text-white hidden md:grid pr-10 lg:pr-4 items-cente grid_teplate_ventas_pendientes gap-4 mb-2 md:px-4 md:py-2  border-y border-gray-300'}
        >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-0 justify-center md:justify-between content_buttons pt-3 mt-5">
            <span className='bg-yellow-400 text-black font-bold h-fit px-2 rounded-md'>Registros: {totalPosts}</span>
          <Paginacion
