  ...
  const filterDate = (): ValuesVentaToMetricas[] => {
    ...
    if (open.fecha) {
      filteredProductos = filteredProductos.filter((pro) => {
        const startDate = parse(pro.fecha_inicio, 'dd/MM/yyyy', new Date())
        const endDate = pro.fecha_fin ? parse(pro.fecha_fin, 'dd/MM/yyyy', new Date()) : null
        if (isValid(startDate) && (!endDate || isValid(endDate))) {
          const formattedStartDate = format(startDate, 'yyyy-MM')
          const formattedEndDate = endDate ? format(endDate, 'yyyy-MM') : null
          return formattedStartDate === open.fecha || (formattedEndDate && formattedEndDate === open.fecha)
        }
        return false
      })
    }
