...
import { DelteItemNewsletter } from '../../../../shared/functions/DelteItemNewsletter'

export const ListarArticulosNesletter = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de articulos - Newsletter')
    Promise.all([
      getDataCategoriasToPortafolio(
        'newsletter/getAllArticulos',
        setProductos,
        setTotalRegistros
      )
    ]).then(() => {
  ...
  const preguntar = (id: number, imagen: string): void => {
    const token = localStorage.getItem('token')
    DelteItemNewsletter({
      ruta: 'newsletter/destroyArticulo',
      id,
      token,
      totalPosts,
      cantidadRegistros,
      paginaActual,
      setpaginaActual,
      rutaFetch: 'newsletter/getAllArticulos',
    ...
    <>
      ...
      {loading
        ...
        <div className="bg-[#fff] p-3 md:p-8 rounded-xl">
          <div className="hidden md:grid grid-cols-1 md:grid-cols-6 gap-4 mb-10 p-4 rounded-md border-b-2 border-solid border-black-50">
            <h5 className="md:text-center text-black">ID</h5>
            <h5 className="md:text-center text-black">Titulo</h5>
            <h5 className="md:text-center text-black">Imagen</h5>
            <h5 className="md:text-center text-black">Categoria</h5>
            <h5 className="md:text-center text-black">Link</h5>
            <h5 className="md:text-center text-black">Opciones</h5>
          </div>
          {filterDate().map((orden: any) => (
            <div
              className="grid grid-cols-1 md:grid-cols-6 gap-2 md:gap-4 items-center mb-4 bg-gray-50 p-4 rounded-xl shadow-sm cursor-pointer relative"
              key={orden.id}
            >
              ...
              <div className="md:text-center md:flex md:justify-center">
                <h5 className="md:hidden text-black font-bold mb-2">Imagen</h5>
                <img
                  src={`https://mysistemalogosperu.s3.amazonaws.com/articulos/${orden.imagen1}`}
                  alt=""
              ...
              <div className="md:text-center md:flex md:justify-center">
                ...
                <p className="line-clamp-2 text-black">{orden.categoria}</p>
              </div>

              <div className="md:text-center md:flex md:justify-center">
                <h5 className="md:hidden text-black font-bold mb-2">Link</h5>
                <a
                  href={`https://logosperu.com.pe/newsletter/${orden.url_categoria?.replace(/^\/|\/$/g, '')}/${orden.url?.replace(/^\/|\/$/g, '')}`}
                  target="_blank"
                ...
                >
                  ...
                  <MenuItem className="p-0 hover:bg-transparent">
                    <Link
                      to={'#'}
                      onClick={() => {
                        preguntar(orden.id, orden.imagen1)
                      }}
                      className="rounded-lg transition-colors text-gray-300 hover:bg-secondary-900 flex items-center justify-center gap-x-4 p-2 flex-1"
                    >
                      Eliminar
