...
import { ModalNewCamapaña } from './components/ModalNewCamapaña'

export const ListaMailsNewsletter = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de mails - Newsletter')
    Promise.all([
      getDataCategoriasToPortafolio(
        'newsletter/getCampanasPrivate',
        setProductos,
        setTotalRegistros
      )
    ]).then(() => {
  ...
  const filterDate = (): ValuesCategoriasPortafolio[] => {
    if (search.length === 0) {
    ...
    <>
      ...
      >
        ...
        <div className="w-full md:w-fit flex flex-col-reverse md:flex-row items-center md:gap-4">
          <Link
            to='/admin/newsletter/mails/reporte'
            className="w-full md:w-fit inline-block rounded bg-main_two md:px-6 pb-2 pt-2.5 text-center text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-main-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-main-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-main-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
          >
            REPORTE
          </Link>
          <button
      ...
      {loading
        ...
        <div className="bg-[#fff] p-3 md:p-8 rounded-xl">
          <div className="hidden md:grid grid-cols-1 md:grid-cols-5 gap-4 mb-10 p-4 rounded-md border-b-2 border-solid border-black-50">
            ...
            <h5 className="md:text-center text-black">Cant. Enviados</h5>
            <h5 className="md:text-center text-black">Cant. Abietos</h5>
            <h5 className="md:text-center text-black">Opciones</h5>
          </div>
          {filterDate().map((orden: any) => (
            <div
              className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-4 items-center mb-4 bg-gray-50 p-4 rounded-xl shadow-sm cursor-pointer relative"
              key={orden.id}
            >
              ...
              <div className="md:text-center md:flex md:justify-center">
                ...
                <p className="line-clamp-2 text-black">{orden.nombre}</p>
              </div>
              ...
              <div className="md:text-center md:flex md:justify-center">
                ...
                <p className="line-clamp-2 text-black">{JSON.parse(orden.correos).length}</p>
              </div>
              ...
              <div className="md:text-center md:flex md:justify-center">
                ...
                <p className="line-clamp-2 text-black">{orden.abiertos ? JSON.parse(orden.abiertos).length : 0}</p>
              </div>
        ...
        <ModalNewCamapaña open={openModal} setOpen={setOpenModal}/>
    </>
  )
}
