...
import { DeleteItemsCategoriasToPortafolio } from '../../../../shared/functions/DeleteItemsCategoriasToPortafolio'

export const ListaCategoriasNewsletter = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de Categorias - Newsletter')
    Promise.all([
      getDataCategoriasToPortafolio(
        'newsletter/getAllCategorias',
        setProductos,
        setTotalRegistros
      )
    ]).then(() => {
  ...
  const preguntar = (id: number): void => {
    ...
    DeleteItemsCategoriasToPortafolio({
      ruta: 'newsletter/destroyCategoria',
      id,
      token,
      totalPosts,
      cantidadRegistros,
      paginaActual,
      setpaginaActual,
      rutaFetch: 'newsletter/getAllCategorias',
    ...
    <>
      ...
      {loading
        ...
        <div className="bg-[#fff] p-3 md:p-8 rounded-xl">
          ...
          {filterDate().map((orden: ValuesCategoriasPortafolio) => (
            <div
