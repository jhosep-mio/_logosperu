...
export const ModalCitaView = ({
  ...
  const saveCita = async (): Promise<void> => {
    if (auth.id != open?.evento?.event.user_id) {
      toast.warning('No puedes actualizar los datos de otro miembro')
      return
    }
    if (values.tipo == 'reunion' && values.miembros.length == 0) {
      toast.warning('No se puede enviar porque la reunion no tiene destinatarios')
      return
    }
    setLoading(true)
    ...
    data.append('miembros', values.miembros && values.tipo == 'reunion' && values.miembros.length > 0 ? JSON.stringify(values.miembros) : '')
    ...
    try {
      const respuesta = await axios.post(
        `${Global.url}/updateCalendarCita/${open?.evento?.event?.orden?.id}`,
        data,
        {
          headers: {
  ...
  const eliminarRegistro = async (): Promise<void> => {
    const token = localStorage.getItem('token')
    if (!open?.evento?.event?.orden?.id) {
      toast.error('No se puede eliminar el registro porque no se encontró el ID del evento.')
      return
    }
    try {
      const confirmacion = await confirmarEliminar()
      if (!confirmacion) return // Si el usuario cancela, no se hace nada
      const respuesta = await axios.delete(`${Global.url}/destroyCalendarCita/${open?.evento?.event?.orden?.id}`, {
        headers: {
          Authorization: `Bearer ${token ?? ''}`
        }
      })
      if (respuesta.data.status === 'success') {
        toast.success(`${values.tipo} eliminado correctamente`)
        getUsuarios()
      } else {
  ...
  useEffect(() => {
    if (toEmail.length > 0) {
      const filteredSuggestions = usuarios.filter((user: any) => {
        ...
        const isMatchingEmail = user.email.toLowerCase().includes(toEmail.toLowerCase())
        const isMatchingName = user.name.toLowerCase().includes(toEmail.toLowerCase())
        const isNotMember = !values.miembros.some((member: any) => member.email === user.email)
        return (isMatchingEmail || isMatchingName) && isNotMember && isNotCurrentAuthUser
      })
    ...
    >
      {!loading
        ...
        <section className="w-full pt-2 flex flex-col">
          ...
          <div className="flex items-center w-full bg-white border-y h-[90px] z-[2]">
            <div className="flex items-center border-r border-gray-200 ">
              {values.tipo == 'cita' && (
                <button
                type='submit'
                form='yourFormId2' // Asigna el ID del formulario aquí
          ...
          <form
            id='yourFormId2' // Asigna un ID al formulario
          ...
          >
            <section className="flex gap-3 w-full">
              ...
              <section className={cn('flex flex-col gap-1 ', values.tipo == 'reunion' ? 'section_citas' : 'w-full')}>
                {values.tipo == 'reunion' && (
                  <>
