...
export const ListadoLandingPage = (): JSX.Element => {
  ...
  useEffect(() => {
    setTitle('Listado de Briefs - Landing Page')
    roles.map(
      (rol: RolsValues): void =>
        rol.id == auth.id_rol &&
        JSON.parse(rol.accesos).map(
          async (route: { peso: string }): Promise<void> => {
            if (route.peso == 'superusuario') {
              await Promise.all([
                getDataNew('getBriefsLanding', setProductos, setTotalRegistros)
              ]).then(() => {
                setLoading(false)
              })
            } else {
              await Promise.all([
                getDataNew(
                  `getBriefsLandingAsignacion/${auth.id}`,
                  setProductos,
                  setTotalRegistros
                )
              ]).then(() => {
              ...
              <div className="md:text-center md:flex md:justify-center items-center">
                ...
                >
                  <MenuItem className="p-0 hover:bg-transparent">
                    {orden.id != null && (
                      <Link
                        to={`/admin/lista-briefs-landing/view/${orden.id}`}
